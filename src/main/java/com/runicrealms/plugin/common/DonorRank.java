...
public enum DonorRank {
    ...
    public static boolean hasRankOrHigher(Player player, DonorRank rank) {
        for (DonorRank target : values()) {
            if (target.priority >= rank.priority && player.hasPermission(rank.permission)) return true;
        }
        return false;
    }

    public static DonorRank getFromIdentifier(String identifier) {
        for (DonorRank rank : values()) {
